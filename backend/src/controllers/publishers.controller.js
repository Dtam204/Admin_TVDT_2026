/**
 * Publishers Controller - Professional Structure
 * Optimized for TEXT columns and Multi-language support
 */
...
exports.update = async (req, res, next) => {
  ...
  try {
    ...
    if (data.name !== undefined) {
      ...
      params.push(typeof data.name === 'string' ? data.name : (data.name?.vi || ''));
    }

    if (data.description !== undefined) {
      updateFields.push(`description = $${paramIndex++}`);
      params.push(typeof data.description === 'string' ? data.description : (data.description?.vi || ''));
    }
